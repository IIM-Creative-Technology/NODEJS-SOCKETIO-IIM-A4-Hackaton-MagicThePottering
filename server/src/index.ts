...
const gameBoardPlayer1 = new Board("player1");
const gameBoardPlayer2 = new Board("player2");

// à déplacer au moment du join dans la room
games.set("laSuperGame", new Game(Steps.BEGIN,[gameBoardPlayer1, gameBoardPlayer2]));
app.get('/init-game', async (req: Request, res: Response) => {
    ...
    await fetch("https://hp-api-iim.azurewebsites.net/matches/start", {
        ...
        body: JSON.stringify({
            ...
            "userIds": [
                game.boards[0].api_id,
                game.boards[1].api_id
            ],
            "type": "1v1"
        })
    })
